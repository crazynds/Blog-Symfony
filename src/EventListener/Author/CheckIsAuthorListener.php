    ...
    public function onKernelController(FilterControllerEvent $event)
    {
        ...
        if (!$author && $this->session->get('pending_user_is_author')) {
            $this->session->getFlashBag()->add(
                'warning',
                'O seu acesso está a ser definido, tente novamente em alguns instantes.'
            );
            ...
            $this->session->getFlashBag()->add(
                'warning',
                'Você não pode acessar a sessão de autor enquanto não for um autor. Porfavor, complete o formulário de ator.'
            );
        }
